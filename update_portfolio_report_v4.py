...
def get_return_pct(ticker, start_date, end_date):
    ...
    try:
        ...
        close = data["Close"]
        return float((close.iloc[-1] / close.iloc[0] - 1) * 100.0)
    except Exception:
        return np.nan
...
# --------------------------------------------------------------
# 1️⃣1️⃣  TICKER & ASSET ALLOCATION PIES (LIKE BEFORE)
# --------------------------------------------------------------
# Ticker pie
...
add_table(
    ["Ticker", "Asset Class", "Shares", "Price ($)", "Value ($)", "Allocation %"],
    ticker_rows,
...
p = doc.add_paragraph("Figure 1: Allocation by individual ticker based on live prices.")
...
p = doc.add_paragraph("Figure 2: Allocation by asset class based on live prices.")
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
